...
/**
 * This class provides an attribute value encoder for encoding arbitrary attribute data to
 * ASN.1 value objects in X.509 certificate attributes.
 *
 * <p>This class builds on an existing encoder implementing the {@link X500NameStyle} interface.
 * By default the {@link CertAttributes} implementation is used.
 * </p>
 *
...
public class AttributeValueEncoder {
   ...
   */
  public void setEncoder(X500NameStyle encoder) {
   ...
   *
   * <p>The exact semantics of the input and the encoded result is determined by the registered attribute encoder.</p>
   * <p>The default encoder follows the following rules</p>
   * <ul>
   *   <li>The attributes e-mail address and domain component are encoded as IA5String</li>
   *   <li>The attribute serialNumber is encoded as printableString if all characters are compatible with
   *   printable string, else it is encoded as UTF8String (in violation with X520, but generally accepted)</li>
   *   <li>Country and DNQualifier are encoded as printableString</li>
   *   <li>DateOfBirth is using the format YYYMMDD, e.g. for 1952-10-05 specify 19521005 and encodes this as GeneralizedTime</li>
   * </ul>
   *
   ...
   */
  public ASN1Encodable encode(ASN1ObjectIdentifier oid, Object value) throws IOException {
    ...
    if (oid.equals(CertAttributes.DATE_OF_BIRTH)) {
      ...
      if (value instanceof String) {
        ...
        if (strValue != null && strValue.matches(DATE_SIMPLE_FOMRAT_REGEX_HYPHEN) || strValue.matches(DATE_SIMPLE_FOMRAT_REGEX)) {
    ...
    try {
      ...
      return encoder.stringToValue(oid, strValue);
    }
    catch (Exception ex) {
