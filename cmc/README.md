...
The main difference between these classes of requests is that the requests from an authorized RA relies purely on standard CMC request and response data defined in RFC 5272 while requests for additional administrative operations makes use of custom request and response data objects defined for this implementation profile.
...
The reason for this particular set of functions is that the scope of this implementation is limited to a specific usecase where the RA acts as the exlusive entity that is authorized to request or revoke certificates on behalf of certificate subjects. The certificate subject has no direct contact with the CA in this scenario. As such we eliminate all need for a state machine and callback scenarios with multiple entities where for example one entity requests the certificate and another entity approves the certificate or similar. This reduces the protocol to a simple request/response protocol where the final result is delivered directly as a response to any supported request.
...
All requests for administrative operations makes use of the regInfo control attribute to hold custom request data and uses the responseInfo control attriubte to return data in responses related to these custom requests.
...
The following table illustrate what data objects that are passed as reqauest and response database

AdminRequestType | Request data | Respone data
---|---|---
...
This CMC API integration library provides java classes that can be used by both the RA and the CA to implement this API.

### CA integration

A API for the CA is implemented using the interface class se.swedenconnect.ca.cmc.api.CMCCaApi

This interface is implemented by the AbstractCMCCaApi and by the AbstractAdminCMCCaApi classes where AbstractCMCCaApi holds basic functioins that should be valid for any implementation and where AbstractCMCCaApi adds a typical implementation of all custom admin functions. Finaly there is a complete default implementation. The class DefaultCMCCaApi. The complete implementation must also provide functions for generating the final certificate content of any issued certificates.
...
The following illustrates typical instatiations of these classes:
...
Four different implementation of CMCRequestModel are provided:
